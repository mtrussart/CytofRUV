...
```{r load and cluster data before CytofRUV normalisation}
  ...
  if (dir.exists(output_dir)) {
    ...
    warning("The output folder CytofRUV_output does not exist, please recreate the original folder.")
  }
...
The normalize_data function allow the user to adjust for batch effects with parameter settings for the CytofRUV algorithm to be selected by user, such as the replicated samples to use, the clusters to be nornmalised and the value of k. The user can specify below the name of the directory to save the normalised data (dir_name_norm_data).
