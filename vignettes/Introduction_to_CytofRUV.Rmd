...
We provided an example dataset to help you prepare your data. The command below will
load all the dataset including the metadata file, the panel file and the fcs files in the working directory.

```{r loading example dataset}
  wd_data="/Users/trussart.m/WEHI/data_raw/"
  write.FCS(x=CytofRUV::A1,filename = paste(wd_data,"A1.fcs",sep=""))
  write.FCS(x=CytofRUV::A2,filename = paste(wd_data,"A2.fcs",sep=""))
  write.FCS(x=CytofRUV::Run3_A1,filename = paste(wd_data,"Run3_A1.fcs",sep=""))
  write.FCS(x=CytofRUV::Run3_A2,filename = paste(wd_data,"Run3_A2.fcs",sep=""))
  write_xlsx(x=CytofRUV::md,path = paste(wd_data,"Metadata.xlsx",sep=""))
  write_xlsx(x=CytofRUV::panel,path = paste(wd_data,"Panel.xlsx",sep=""))
```

## Loading datasets

The first step is to load the metadata file, all the .fcs files, the panel file from the dataset. The user needs to define the working directory as well as the filename of the metadata file and the panel file. Once the dataset is loaded it will then cluster all the data. The user needs to provide the number of clusters to be used for this dataset.
...
```{r ShinyApp on the raw data}
  ...
  sub_daf_state <- daf_type[, sample(ncol(daf_type), n_subset_marker_specific)]
...
The normalize_data function allow the user to adjust for batch effects with parameter settings for the CytofRUV algorithm to be selected by user, such as the replicated samples to use, the clusters to be nornmalised and the value of k. 
...
```{r load and cluster data after CytofRUV normalisation}
  ...
  ## Loading the data
  norm_data=load_data(wd_norm,metadata_norm_filename,panel_norm_filename)
...
``` {r Shiny App on the normalised data}
  ## Define parameters to use for R-Shiny
  daf=norm_data$daf
  md=norm_data$md
  seed=1234
